...
//        variances.emplace_back(((det / trace) / outliers_garde) * z.size());
    ...
    /*z = Auxiliary::getZValues(weightedPoints.front().second);
    double zsig = std::sqrt(Auxiliary::calculateVariance(z));
    double zMean = Auxiliary::calculateMean(z);
    double upperBound = zsig + zMean;
    double lowerBound = -zsig + zMean;
    std::vector<Point> full = weightedPoints.front().second;
    for (int i = 0; i < weightedPoints.front().second.size(); ++i) {
        auto point = weightedPoints.front().second[i];
        if (upperBound < point.z || lowerBound > point.z) {
            weightedPoints.front().second.erase(weightedPoints.front().second.begin() + i);
            i--;
        }
    }*/
